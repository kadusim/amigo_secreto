...
(function() {
  ...
  $(document).on('turbolinks:load', function() {
    ...
    return $('.member_change').on('change', 'div.member_id', function(e) {
      $.ajax('/members/' + e.currentTarget.id, {
        type: 'PUT',
        dataType: 'json',
        data: {
        ...
        success: function(data, text, jqXHR) {
          return Materialize.toast('Membro atualizado', 4000, 'green');
        },
        error: function(jqXHR, textStatus, errorThrown) {
          return Materialize.toast('Problema na atualização da Membro', 4000, 'red');
        }
      });
      return false;
    });
  ...
  insert_member = function(id, name, email) {
    return $('.member_list').append('<div class="member_change" id="member_' + id + '">' + '<div class="row" id="' + id + '">' + '<div class="col s12 m5 input-field">' + '<input id="name_' + id + '" type="text" class="validate" value="' + name + '">' + '<label for="name" class="active">Nome</label>' + '</div>' + '<div class="col s12 m5 input-field">' + '<input id="email_' + id + '" type="email" class="validate" value="' + email + '">' + '<label for="email" class="active" data-error="Formato incorreto">Email</label>' + '</div>' + '<div class="col s3 offset-s3 m1 input-field">' + '<i class="material-icons icon">visibility</i>' + '</div>' + '<div class="col s3 m1 input-field">' + '<a href="#" class="remove_member" id="' + id + '">' + '<i class="material-icons icon">delete</i>' + '</a>' + '</div>' + '</div>' + '</div>');
  };
